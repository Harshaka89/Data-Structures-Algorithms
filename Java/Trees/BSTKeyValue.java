...
public class BST<Key extends Comparable<Key>, T> {
	public Node root;
	public class Node{
		Key key;
		T data;
	...
	private T get(Node current, Key key){
		if(current == null){
			return null;
		}
	...
	private Node put(Node current, Key key, T value){
	...
	private Node delete(Node current, Key key){
		...
		if(cmp < 0){ //if key smaller than current node go left
			...
			Node t = current; //saves link to the node which will be deleted
			current = min(t.right); //sets the node to be deleted to its successor t.right
	...
	/* Methods to return the largest key in the table less than or equal to the key passed in */
	public Key floor(Key key){
		if(isEmpty()){
			throw new NoSuchElementException("Called floor() when BST is empty!");
		}
	...
	/* Methods to return the smallest key in the table greater than or equal to the key passed in */
